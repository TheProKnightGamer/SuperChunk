...
public class DynamicPriorityQueue<E> {
    ...
    /**
     * SuperChunk player.latencyMetrics: weakly consistent snapshot of the queued-element count
     * per priority (index = priority). Reads the existing {@link #taskCount} atomics only — no
     * locking, no effect on queue behavior.
     */
    public int[] taskCountSnapshot() {
        final int[] counts = new int[this.priorities.length];
        for (int i = 0; i < counts.length; i++) {
            counts[i] = this.taskCount.get(i);
        }
        return counts;
    }
