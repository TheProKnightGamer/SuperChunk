...
import dev.superchunk.com.ishland.c2me.opts.dfc.common.ast.dfvisitor.StripBlending;
...
public class BytecodeGen {
    ...
    private static final boolean DUMP_CLASSES = Boolean.getBoolean("superchunk.dfc.dumpClasses");
    ...
    public static DensityFunction compile(DensityFunction densityFunction, Reference2ReferenceMap<DensityFunction, DensityFunction> tempCache) {
        ...
        if (densityFunction instanceof AstVanillaInterface vif) {
            ...
            CompiledDensityFunction compiledVif = new CompiledDensityFunction(compile0(ast), vif.getBlendingFallback());
            // SuperChunk COMPACT-IDS (Stage 5): stash the source AST so the route capture
            // (CompactIds.captureRouter) can read it even when the DF has no GPU form.
            compiledVif.c2me$setSourceAst(ast);
            // SuperChunk GPU (Stage 2): attach a GPU-compiled form when enabled.
            dev.superchunk.gpu.dfc.GpuDfcHook.maybeAttach(ast, compiledVif);
            return compiledVif;
        }
        ...
        // SuperChunk COMPACT-IDS (Stage 5): stash the source AST so the route capture
        // (CompactIds.captureRouter) can read it even when the DF has no GPU form (the
        // full finalDensity contains McToAst-opaque nodes and never GPU-compiles whole;
        // its interpolated subtrees do). Pure reference stash — no behavior change.
        compiled.c2me$setSourceAst(ast);
        // SuperChunk GPU (Stage 2): attach a GPU-compiled form when enabled.
        dev.superchunk.gpu.dfc.GpuDfcHook.maybeAttach(ast, compiled);
...
//        genFields(genContext);
        ...
        if (DUMP_CLASSES) dumpClass(genContext.className, bytes);
