 ...
 * <ul>
 *   <li>{@code -Dsuperchunk.worldgen.jigsawOctree} — use the octree fast path.</li>
 *   <li>{@code -Dsuperchunk.worldgen.jigsawOctree.verify} — run BOTH the octree and the real
 *       VoxelShape path, assert they agree, and (crucially) RETURN THE VANILLA RESULT so output
 *       stays bit-identical while we prove correctness. Logs any mismatch as a BUG.</li>
 * </ul>
 *
...
public final class JigsawShapeTracker {
    ...
    public static boolean testFree(VoxelShape free, VoxelShape candidate, BooleanOp op) {
        ...
        if (VERIFY) {
            ...
            maybeLog();
            return real; // keep output bit-identical to vanilla in verify mode
        }
        if (fast) STUCK.incrementAndGet();
        maybeLog();
        return fast;
    }
