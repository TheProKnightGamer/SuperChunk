package dev.superchunk.com.ishland.c2me.opts.dfc.common.ast.spline;
...
public class SplineAstNode implements AstNode {
    ...
    public double evalSingle(int x, int y, int z, EvalType type) {
        return spline.apply(new DensityFunctions.Spline.Point(new NoisePosVanillaInterface(x, y, z, type)));
    }
    ...
    private static ValuesMethodDef doBytecodeGenSpline(BytecodeGen.Context context, CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> spline) {
        ...
        if (spline instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> spline1) {
        ...
        if (spline instanceof CubicSpline.Multipoint<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> impl) {
            ...
            String locationFunction = context.newSingleMethod(McToAst.toAst(impl.coordinate().function().value()));
        ...
        } else if (spline instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> floatFunction) {
    ...
    private static boolean deepEquals(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a,
        ...
        if (a instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a1 &&
            ...
            boolean equals1 = Arrays.equals(a1.derivatives(), b1.derivatives()) &&
                    ...
                    McToAst.toAst(a1.coordinate().function().value()).equals(McToAst.toAst(b1.coordinate().function().value()));
    ...
    private static boolean deepRelaxedEquals(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a,
        ...
        if (a instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a1 &&
            ...
            boolean equals1 = a1.values().size() == b1.values().size() &&
                    McToAst.toAst(a1.coordinate().function().value()).relaxedEquals(McToAst.toAst(b1.coordinate().function().value()));
            if (!equals1) return false;
            int size = a1.values().size();
            for (int i = 0; i < size; i++) {
    ...
    private static int deepHashcode(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a) {
        if (a instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a1) {
            ...
            result = 31 * result + McToAst.toAst(a1.coordinate().function().value()).hashCode();
    ...
    private static int deepRelaxedHashcode(CubicSpline<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a) {
        if (a instanceof CubicSpline.Constant<DensityFunctions.Spline.Point, DensityFunctions.Spline.Coordinate> a1) {
            ...
            result = 31 * result + McToAst.toAst(a1.coordinate().function().value()).relaxedHashCode();
